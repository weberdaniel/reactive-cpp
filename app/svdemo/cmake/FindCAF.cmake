...
# set before calling find_package): # #  CAF_ROOT_DIR  Set this variable either to an 
# installation prefix or to wa CAF build directory where to look for the CAF libraries. 
...
foreach (comp ${CAF_FIND_COMPONENTS})   
  ...
  if (CAF_ROOT_DIR)
    set(header_hints "${CAF_ROOT_DIR}/include"
  ...
  if (NOT "${CAF_INCLUDE_DIR_${UPPERCOMP}}"       STREQUAL "CAF_INCLUDE_DIR_${UPPERCOMP}-NOTFOUND")
  ...
  if (NOT ${comp} STREQUAL "probe_event" AND NOT ${comp} STREQUAL "test")
    if (CAF_ROOT_DIR)
      set(library_hints "${CAF_ROOT_DIR}/lib"
	                "${CAF_ROOT_DIR}/libcaf_core"
	                "${CAF_ROOT_DIR}/libcaf_io"
	                "${CAF_ROOT_DIR}/libcaf_net"
	                "${CAF_ROOT_DIR}/libcaf_openssl")
    endif ()
      ...
      if ("${CAF_LIBRARY_${UPPERCOMP}}"           STREQUAL "CAF_LIBRARY_${UPPERCOMP}-NOTFOUND")
        set(CAF_${comp}_FOUND false)
      else ()
...
# let CMake check whether all requested components have been found include(FindPackageHandleStandardArgs)
find_package_handle_standard_args(CAF
  FOUND_VAR CAF_FOUND REQUIRED_VARS CAF_LIBRARIES CAF_INCLUDE_DIRS HANDLE_COMPONENTS)
